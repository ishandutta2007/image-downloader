        ...
        for p in r_post.json()['photos']:
            ...
            if not p['watermark']:
                try:
                    ...
                    print e
            print username,popular,p['watermark'],(p['favorites_count']), p['highest_rating'] ,  p['url']

            if username not in zero_fav_enable:
                if not popular and (p['watermark'] or (  p['highest_rating'] < 92 )):
                    continue
            ...
            if category_id_list and category_id_list[0] != '' and str(category) not in category_id_list:
                print  category_id_list,category
                continue
            print  'vvv'
